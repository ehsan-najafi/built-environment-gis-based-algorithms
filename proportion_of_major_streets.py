'''----------------------------------------------------------------------------------
 Name: Proportion of Major Streets
 Source: proportion_of_major_streets.py
...
in_street_fc = r"D:\gis-based-built-environment-algorithms\data_test\sample_data.gdb\sample_streets"
...
### Create copy featureclass for Streets
in_street_fc_temp = os.path.join(env_path, "in_street_fc_temp.shp") 
if arcpy.Exists(in_street_fc_temp):
	arcpy.Delete_management(in_street_fc_temp)
arcpy.FeatureClassToFeatureClass_conversion(in_street_fc, os.path.dirname(in_street_fc_temp), os.path.basename(in_street_fc_temp), "")

# ### Dissolve
# out_pedestrain_roads_diss = gdb_temp_path + "\\" + "out_pedestrain_roads_diss"
# if arcpy.Exists(out_pedestrain_roads_diss):
#	 arcpy.Delete_management(out_pedestrain_roads_diss)
# arcpy.management.Dissolve(in_street_fc_temp, out_pedestrain_roads_diss, "", "", "")

# ### Feature To Line
# out_pedestrain_roads_fin = env_path + "\\" + "out_pedestrain_roads_fin.shp"
# if arcpy.Exists(out_pedestrain_roads_fin):
#	 arcpy.Delete_management(out_pedestrain_roads_fin)
# arcpy.FeatureToLine_management(out_pedestrain_roads_diss, out_pedestrain_roads_fin, "", "ATTRIBUTES")

### Create New Field 'Majority_Street_Index' in Sample Points Feature Class
Majority_Street_Index_field = "Majority_Street_Index_" + str(buffer_in_meters) + "m"
if str(arcpy.ListFields(in_sample_points, Majority_Street_Index_field)) == "[]":
	arcpy.AddField_management(in_sample_points, Majority_Street_Index_field, "DOUBLE")
...
### Cacluate 'Majority_Street_Index'
for selected_id in list_ids:
	...
	if arcpy.Describe(in_service_area_lyr).FIDSet != '':
		### Clip Street Features inside sample point network buffer
		out_clip_roads = gdb_temp_path + "\\" + "clip_road_" + str(selected_id)
		arcpy.Clip_analysis(in_street_fc_temp, in_service_area_lyr, out_clip_roads, "")

		### Cacluate Index
		sum_total_length = 0
		sum_major_length = 0
		with arcpy.da.SearchCursor(out_clip_roads, ["Shape_Length", "IS_PRIMARY"]) as cursor:
			for row in cursor:
				sum_total_length += row[0]
				if row[1] == 1:
					sum_major_length += row[0]

		### Update Field 'Majority_Street_Index' in Sample Points Feature Class
		with arcpy.da.UpdateCursor(in_sample_points, [Majority_Street_Index_field], "ID = " + str(selected_id)) as UC:
			for row in UC:
				row[0] = float(sum_major_length / sum_total_length)
				UC.updateRow(row)

		### Remove Temporary Layers
		arcpy.Delete_management(out_clip_roads)	
